...
// This function now prints the node's COLOR instead of balance
void printIntWithColor(void *data, void *extra_data) {
    (void) extra_data;
    Tree node = (Tree)((char *)data - offsetof(struct _TreeNode, data));
    int value = *(int*)data;
    // Print "RED" or "BLACK" based on the color enum
    printf("Valeur : %2d (Color: %s)\n", value, node->color == RED ? "RED" : "BLACK");
}

// You can update the other print functions similarly if you want
void printStrWithColor(void *data, void *extra_data) {
    (void) extra_data;
    Tree node = (Tree)((char *)data - offsetof(struct _TreeNode, data));
    char *value = *(char **)data;
    printf("Valeur : %-10s (Color: %s)\n", value, node->color == RED ? "RED" : "BLACK");
}
...
void printEntry(void *data, void *extra_data) {
    ...
    Entry *e = (Entry *)data;
    printf("{%-7s : %-18s} (COLOR: %s)\n", e->mot, e->definition, node->color == RED ? "RED" : "BLACK");
}
...
void testAVLInt(void) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        tree_post_order(root, printIntWithColor, NULL);
        printf("\n");
    }
...
void testAVLStr(void) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        printf("Post-order après insertion :\n");
        tree_post_order(root, printStrWithColor, NULL);
        printf("\n");
    }
